define(['3rd/pako/pako_inflate.min'],function(pako){'use strict';
  ...
  function downloadData(fileinfo){
    ...
    ursTxtFileList=fileinfo.urstxtfilelist.slice(0);
    resovedBinFiles=[];
    resovedTxtFiles=[];
    dataUint8=[];
    dataInt32=[];
  ...
  function queryResovedTxtFiles(){
    return resovedTxtFiles;
  };
  ...
  function _resolveTarFile(){
    ...
    for(var offset=0,len=dataUint8.length;offset<len;offset+=512){
      ...
      var i=0,
          ...
          numut=ursTxtFileList.length;
      while(i<numu){
        if(numuu===0&&numut===0){
          //console.log("all bin & text files located");
          return;
        }
        ...
        if(tempfilename.indexOf(ursFileList[i])===0){
          ...
          for(var j=0;j<numut;++j){
            if(ursFileList[i]===ursTxtFileList[j]){
              ursTxtFileList.splice(j,1);
              --numut;
              _resolveTxtFile(ursFileList[i],tempfilesize,offset+512);
            }
          }
  ...
  function _resolveTxtFile(filename,filesize,offset){
    var uint8result=new Uint8Array(dataUint8.buffer,offset,filesize);
    var txtresult="";
    for(var i=0;i<filesize;++i){
      txtresult+=String.fromCharCode(uint8result[i]);
    }
    var txtfile={
      filename: filename,
      content: txtresult
    }
    resovedTxtFiles.push(txtfile);
  }
  
  return{
    downloadData: downloadData,
    queryResovedBinFiles: queryResovedBinFiles,
    queryResovedTxtFiles: queryResovedTxtFiles
  };
