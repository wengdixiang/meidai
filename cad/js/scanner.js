...
$index=0; //多用户登陆页面索引
$_page=0;//多用户登陆页面页
$registration_index=0;//多用户注册页面索引
$registration_page=0;//多用户注册页面页
$form=$("form");
$content=$("#content");
$panduan=$(".panduan",$content);
$loginForm=$("#login_form");
$emailPhone=$(".email_or_phone",$loginForm);//登录页面邮箱电话
...
$email_or_phone_error=$(".email_or_phone_error",$loginForm);
$registrationForm=$("#registration_form");
$emailByphone=$(".email_by_phone",$registrationForm);
$registration_error=$(".registration_error",$registrationForm);

$registrationFormPassword=$(".password",$registrationForm);
// $flexBox=$(".flex-box");
// $comRegistration=$("#com_registration");  //继续注册按钮

$userLogin=$("#user_login");
$loginBox=$(".login_box");
$loginBoxDiv=$("div",$loginBox);
$user_registration=$("#user_registration");
$userName="";
$user=$(".user");
$username=$(".select_userName",$user);
$li.each(function(){
...
//登陆界面
 ...
 $loginForm.on("submit",function(event){
 	// function login_submit(ev){
	    ...
	    $loginUserName=$("#login_form .login_form_userName input[type='text']").val();
		//$registrationFormPasswordValue=$registrationFormPassword.val();
	    if($emailPhoneVal==""){
...
function goTohtml(){
  ...
  if(useruuid !== ''){
    ...
    $.ajax({
      url: "/user", type: 'POST',
      data: {'action': "query", 'uuid': useruuid, 'username': '', 'lastorder': ''},
      success: function(data) {
        var userdata=JSON.parse(data);
        username=userdata.username;
        orderuuid=userdata.lastorder;
        if(orderuuid.length===0){
          newOrderToScanner(useruuid,username);
        }
        else{
          startScanner(orderuuid,username);
        }
      },
      error: function(err) {
        alert("请求用户数据错误："+err);
...
function newOrderToScanner(useruuid,username){
  console.log("newOrderToScanner");
  //here create a new order
  $.ajax({
    url: "/order", type: 'POST',
    data: {'action': "create", 'owneruuid': useruuid},
    success: function(data) {
      var orderdata=JSON.parse(data);
      console.log(orderdata);
      if(orderdata.result==="true"){
        console.log("order query result: true");
...
function startScanner(orderuuid,username){
  console.log("start scanner");
  var url="fswscanner://"+document.domain+"/data?action=upload&type=rawscan&owneruuid="+orderuuid+"&username="+username;
  console.log(url);
  location.href = url;
};
//-----------------------------
