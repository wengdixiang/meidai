$(function(){
        ...
        for(var i in deal_list){
            if(!all_deal_data[deal_list[i]]){                        
                ...
                case 'topay':
                  ...
                  tmp_headform_class = 'display_block';
                  tmp_headform_receipts = '参数调整';
                  break;
                case 'paid':
                  order_status = '已支付';
                  tmp_button = '查看详情';
                  ...
                  tmp_headform_class ='display_none';
                  tmp_headform_receipts = '';
                  break;  
                // default:
                //   order_status = '待支付';
                //   tmp_button = '提交订单';
                ...
                table.row.add([
                    ...
                    "<button id='headform' class='btn_info btn-sm "+tmp_headform_class+"' alternews ='"+deal_list[i]+"'>"+tmp_headform_receipts+"</button>",                
    ...
    // 
    $('#table_id_example tbody').on('click','#headform',function(){
        var dealuuid = $(this).attr('alternews');
        setCookie("deal",dealuuid,0.5);
        console.log(getCookie("deal"));
        window.location.href = location.href+"sjy/glassadmin.html";
        // window.location.href = "https://leixing.fswear.local/tast/www8/sjy/glassadmin.html";
    });
        ...
        function personal_information(){
            ...
            function insert_data_to_page(){
                ...
                if(!glass_config_data.SlotMessage){
                    $('.frame_lettering input').eq(0).val('');
                    $('.frame_lettering input').eq(1).val('');
                }else if(glass_config_data.SlotMessage.length >= 4){
                    $('.frame_lettering input').eq(0).val(glass_config_data.SlotMessage[0]);
                    $('.frame_lettering input').eq(1).val(glass_config_data.SlotMessage[2]);
                }else if(glass_config_data.SlotMessage.length == 1 || glass_config_data.SlotMessage.length == 2){
                    $('.frame_lettering input').eq(0).val(glass_config_data.SlotMessage[0]);
                    $('.frame_lettering input').eq(1).val('');
                }else if(glass_config_data.SlotMessage.length == 3){
                    $('.frame_lettering input').eq(0).val(glass_config_data.SlotMessage[0]);
                    $('.frame_lettering input').eq(1).val(glass_config_data.SlotMessage[1]);
                }
            	
                if(!glass_config_data.LegMessage){
                    $('.leg_lettering input').eq(0).val('');
                    $('.leg_lettering input').eq(1).val('');
                    $('.leg_lettering input').eq(2).val('');
                }else{
                    if(glass_config_data.LegMessage.length == 2){
                        $('.leg_lettering input').eq(0).val(glass_config_data.LegMessage[0]);
                        $('.leg_lettering input').eq(1).val(glass_config_data.LegMessage[1]);
                        $('.leg_lettering input').eq(2).val('');
                    }else{
                        $('.leg_lettering input').eq(0).val(glass_config_data.LegMessage[0]);
                        $('.leg_lettering input').eq(1).val(glass_config_data.LegMessage[1]);
                        $('.leg_lettering input').eq(2).val(glass_config_data.LegMessage[2]);
                    }
                }
                ...
                if(!glass_config_data.LegColor)	{
                    ...
                    if(tmp_leg_color == ''){
                        ...
                        $('.glass-leg-color input').val('#'+tmp_leg_color);
                ...
                if(!glass_config_data.LensProfileFile){
                    ...
                    if(tmp_lensprofilefile.slice(0,2) === 'KM'){
                        ...
                        $('.glass-frame>div').append('<img src="images/information/frame18.png">');
	...
	$('#submit_preserve,#submit_order').click(function(){
        ...
        if($('.glass-frame-color input').val() != ''){
            var tmp_frame_color = $('.glass-frame-color input').val();
        ...
        if($('.glass-leg-color input').val() != ''){
            var tmp_leg_color = $('.glass-leg-color input').val();
