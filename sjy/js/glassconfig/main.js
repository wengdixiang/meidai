(function(){'use strict';
  ...
  require(['js/glassconfig/headloader','js/glassconfig/glassloader','js/glassconfig/display','js/glassconfig/scrolleffect','js/glassconfig/cookie','js/glassconfig/framedesign','js/glassconfig/legdesign','js/glassconfig/color','js/glassconfig/fileurlgenerator','js/glassconfig/progress'],function(headloader,glassloader,display,scrolleffect,cookie,framedesign,legdesign,color,fileurlgenerator,progress){
    ...
    var minInterfaceWidth=433,
        ...
        idTimerDownloadConfig=null,
        downloadConfigInterval=200,
    ...
    //start download config
    $('#button-param-download').click(function(){
      console.log("param download event");
      if(window.confirm('您确定要下载config吗？')){
        $('div.tab-content').css('pointerEvents','none');
        var tempconfig=generateConfigText();
        fileurlgenerator.glassUrlGenerator(tempconfig);
        idTimerDownloadConfig=setInterval(queryDownloadConfig,downloadConfigInterval);
      }
    });
    
    function queryDownloadConfig(){
      console.log("queryDownloadConfig");
      var tempconfiguuid=fileurlgenerator.getPresentConfigUuid();
      if(tempconfiguuid!==""){
        clearInterval(idTimerDownloadConfig);
        var tempconfigurl="/data?action=download&type=config&uuid="+tempconfiguuid;
        window.open(tempconfigurl);
        $('div.tab-content').css('pointerEvents','');
      }
    }
